...
const getIconAndColor = (type: string) => {
  ...
  switch (normalized) {
    case "FRIEND_REQUEST":
    case "SOLICITUD":
      return { icon: <UserPlus size={16} className="text-white" />, color: "bg-indigo-600" };
    case "COMMENT":
    case "COMENTARIO":
      return { icon: <MessageCircle size={16} className="text-white" />, color: "bg-blue-500" };
    case "LIKE":
      return { icon: <Heart size={16} className="text-white" />, color: "bg-red-500" };
    default:
      return { icon: <Bell size={16} className="text-white" />, color: "bg-gray-400" };
  }
};

export const NotificationCard = ({ notification }: { notification: any }) => {
  const { setNotifications } = useNotificationsC();
  ...
  const isRequest = tipo === "FRIEND_REQUEST";
  const bgClass = leido ? "bg-white" : "bg-blue-50/60 border-blue-100";
  ...
  const destination = isRequest ? `/perfil/${actor_username}` : `/itinerario/${linkId}`;

  return (
    <div className={`relative w-full p-3 mb-2 rounded-xl border shadow-sm transition-all hover:shadow-md ${bgClass} border-gray-200`}>
      {/* Usamos un div envolvente en lugar de Link directo para controlar mejor los botones */}
      <div className="flex gap-3 items-start">
        
        {/* 1. AVATAR + ICONO */}
        <Link href={`/perfil/${actor_username}`} className="relative shrink-0 cursor-pointer">
          <Image
          ...
          />
          <div className={`absolute -bottom-1 -right-1 p-1 rounded-full border-2 border-white ${color}`}>
            {icon}
          </div>
        </Link>
        ...
        <div className="flex-1 min-w-0">
          <Link href={destination} className="block">
            <p className="text-sm text-gray-800 leading-snug">
              <span className="font-bold text-gray-900">{actor_nombre}</span> {mensaje}
            </p>
