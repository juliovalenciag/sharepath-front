...
export default function FriendsPage() {
  ...
  const handleRespond = async (id: number, state: number) => {
    try {
      ...
      if (state === FriendRequestState.FRIEND) {
        ...
        loadFriendsData(); // Recargar lista de amigos
      } else {
  ...
  // === NUEVA FUNCIÓN: Enviar solicitud desde sugerencias ===
  const handleConnect = async (username: string) => {
    try {
      // Usamos el mismo método de la API que usaste en "BuscarViajeroPage"
      await api.sendFriendRequest(username);
      
      toast.success(`Solicitud enviada a @${username}`);
      
      // Quitamos la sugerencia de la lista porque ya no es sugerencia, es pendiente
        ...
        <div className="flex items-center justify-between">
          <TabsList className="h-9 w-full justify-start rounded-lg bg-muted p-1 sm:w-auto">
            <TabsTrigger value="friends" className="px-4 text-xs sm:text-sm">
              Amigos
              <Badge variant="default" className="ml-2 h-5 px-1.5 text-[10px] font-normal">
                {totalFriends}
              </Badge>
            </TabsTrigger>
            <TabsTrigger value="requests" className="px-4 text-xs sm:text-sm">
              Solicitudes
              {totalRequests > 0 && (
                <Badge variant="destructive" className="ml-2 h-5 px-1.5 text-[10px] font-normal">
                  {totalRequests}
                </Badge>
              )}
            </TabsTrigger>
            <TabsTrigger value="suggestions" className="px-4 text-xs sm:text-sm">
              Sugerencias
            </TabsTrigger>
  ...
  return (
    <Card className={`overflow-hidden transition-all hover:shadow-md ${active ? "border-primary/50 ring-1 ring-primary/20" : ""}`}>
      <CardContent className="flex items-center gap-4 p-4">
        <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${colors[intent]}`}>
...
function FriendCard({
  ...
  const confirmBlock = () => {
    toast(`¿Estás seguro de bloquear a @${friend.username}?`, {
      ...
      action: { label: "Bloquear", onClick: () => onBlock(friend.username) },
  ...
  return (
    <Card className="group flex flex-col justify-between overflow-hidden transition-all hover:border-primary/30 hover:shadow-md">
        ...
        <div className="space-y-2 rounded-lg bg-muted/40 p-2.5 text-xs text-muted-foreground">
          {(friend.city || friend.country) && (
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">{[friend.city, friend.country].filter(Boolean).join(", ")}</span>
            </div>
          )}
      ...
      <div className="grid grid-cols-2 border-t">
        <Button variant="ghost" className="w-full rounded-none h-10 text-xs text-muted-foreground hover:bg-muted hover:text-foreground border-r" onClick={() => onDelete(friend.correo)}>
          <UserMinus className="mr-2 h-3.5 w-3.5" /> Eliminar
        </Button>
        <Button variant="ghost" className="w-full rounded-none h-10 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={confirmBlock}>
          <Ban className="mr-2 h-3.5 w-3.5" /> Bloquear
        </Button>
      </div>
    </Card>
  );
      ...
      <CardContent className="p-4">
        ...
        <Link href={`/viajero/perfil/${request.username}`} className="flex gap-3 group hover:bg-muted/5 p-1 -m-1 rounded-md transition-colors">
          ...
          <div className="flex-1 space-y-1 min-w-0">
            ...
            {request.message && (
              <div className="mt-2 rounded-md bg-muted p-2 text-xs italic text-muted-foreground">&quot;{request.message}&quot;</div>
            )}
          </div>
        </Link>
      </CardContent>
      <div className="grid grid-cols-2 gap-px bg-border border-t">
        <Button variant="ghost" className="rounded-none bg-card hover:bg-primary/5 hover:text-primary h-10 text-xs font-medium disabled:opacity-50" onClick={() => handleAction(FriendRequestState.FRIEND)} disabled={loading}>
          {loading ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <UserCheck className="mr-2 h-3.5 w-3.5" />} Aceptar
        </Button>
        <Button variant="ghost" className="rounded-none bg-card hover:bg-destructive/5 hover:text-destructive h-10 text-xs font-medium disabled:opacity-50" onClick={() => handleAction(FriendRequestState.REJECTED)} disabled={loading}>
          {loading ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <UserX className="mr-2 h-3.5 w-3.5" />} Rechazar
        </Button>
      </div>
    </Card>
  );
    ...
    <Card className="flex flex-col justify-between transition-all hover:border-primary/40">
      <CardContent className="p-4">
        ...
        <div className="flex flex-wrap gap-1.5 min-h-[24px]">
          {suggestion.interests.slice(0, 3).map((tag) => (
            <Badge key={tag} variant="secondary" className="rounded-md px-1.5 py-0 text-[10px] font-normal">{tag}</Badge>
          ))}
          {suggestion.interests.length > 3 && (
            <Badge variant="outline" className="rounded-md px-1.5 py-0 text-[10px] font-normal">+{suggestion.interests.length - 3}</Badge>
          )}
        </div>
      </CardContent>
      <div className="p-3 pt-0 mt-auto">
        <Button 
          ...
          disabled={loading}
        >
          {loading ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <UserPlus className="mr-2 h-3.5 w-3.5" />}
          Conectar
        </Button>
      </div>
    </Card>
