...
const ResultadoViajero: React.FC<{ data: ViajeroData }> = ({ data }) => {
  ...
  return (
    <div className="flex items-center justify-between p-3 border border-gray-200 rounded-xl bg-white shadow-sm transition duration-150 hover:shadow-md">
      <div className="flex items-center">
        {/* Avatar */}
        <div className="w-12 h-12 rounded-full mr-4 bg-gray-200 flex items-center justify-center text-xl font-medium text-gray-500 overflow-hidden">
          {data.foto_url ? (
            
            <img src={data.foto_url} alt={data.nombre_completo} className="w-full h-full object-cover" />
          ) : (
        ...
        <div>
          <p className="font-semibold text-gray-800">
            ...
            <span className="text-sm font-normal text-gray-500">@{usernameDisplay}</span>
          </p>
...
const SugerenciaAmistad: React.FC<{ nombre: string, status: 'Agregar' | 'Solicitud enviada' }> = ({ nombre, status }) => {
...
export default function BuscarViajeroPage() {
    ...
    const handleSearch = async (term: string) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/user/search?q=${encodeURIComponent(term)}`, {
                method: 'GET',
                headers: {
                    'token': token,
                }
    ...
    return (
        <div className="flex-1 p-6 md:p-10 bg-gray-50 min-h-screen">
            ...
            <div className="flex items-center mb-6">
                <Link href="/viajero" className="text-gray-700 hover:text-gray-900 mr-4 p-2 transition duration-150">
                    <HiArrowLeft size={24} aria-label="Volver atrás" />
                </Link>
                <h1 className="text-3xl font-bold text-gray-800">Buscar viajeros</h1>
            </div>

            {/*  Barra de Búsqueda */}
            <div className="mb-8">
                <div className="relative">
                    <HiOutlineMagnifyingGlass className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
                    <input
                        type="text"
                        placeholder="¡Busca a un amigo viajero!"
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full p-4 pl-12 border border-gray-300 rounded-xl shadow-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 text-gray-700"
                    />
                </div>
            </div>
            ...
            {/* Estado de carga */}
            {loading && (
                <p className="text-center text-blue-500 font-medium my-8">Buscando viajeros...</p>
            )}
            ...
            <div className="space-y-4 mb-12">
                {!loading && viajeros.length > 0 ? (
                    ...
                    !loading && searchTerm.trim() !== '' && (
                        <p className="text-center text-gray-500 my-8">No se encontraron viajeros que coincidan.</p>
                    )
                )}
            </div>
            
            {/* Sugerencias de Amistad  */}
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Sugerencias de amistad</h2>
            <div className="flex overflow-x-auto space-x-4 pb-4">
                ...
                <div className="flex items-center justify-center flex-shrink-0">
                    <button className="p-3 text-2xl text-gray-600 bg-white border border-gray-200 rounded-full shadow-md hover:bg-gray-100 transition duration-150">
                        &rarr;
                    </button>
                </div>
            </div>
