...
const actualizarEstado = async (id: number, nuevoEstado: string) => {
  try {
    ...
    setItinerarios((prev) =>
      prev.map((it) =>
        it.id === id ? { ...it, estado: nuevoEstado } : it
      )
    );
    filtrar();
  } catch (error) {
    ...
    <div className="p-6 flex flex-col gap-10 min-h-screen">
      ...
      <h1 className="text-3xl font-bold">Gestión de Itinerarios</h1>
        ...
        <input
            ...
            <Button onClick={filtrar} className="px-4 py-2 rounded-xl text-sm font-medium">
            Ir
            </Button>
        </div>
      ...
      <Card className="shadow-lg border rounded-xl p-0">
         <CardContent className="p-0 overflow-x-auto w-full">
          <table className="w-full text-sm border-collapse">
            ...
            <tr className="text-left">
                <th className="p-4 border-b rounded-tl-xl">Nombre de usuario</th>
        ...
        {filtrados.length > 0 ? (
            ...
            <tr key={it.id} className="hover:bg-gray-50 transition">
                ...
                <select
                    value={it.estado}
                    onChange={(e) => actualizarEstado(it.id, e.target.value)}
